...
- **Tokens:** os valores escuros ficam em `styles/tokens.css`, em `:root[data-theme='dark']`. É um cinza-carvão em camadas: fundo `#212224`, cards `#28292b`, modais `#2a2b2d`, inputs `#2b2c2e`.
...
- **Proposta:** usa uma versão noturna do vídeo (`propostaDark` em `assets.ts`), com as mesmas qualidades adaptativas.
  ...
  - Para regerar o vídeo com a correção:
    ...
    ```bash
    npm run compress:video -- original.mp4 public/assets/video/proposta-car-dark-1280.mp4 1280 27 "colorlevels=romin=0.05:gomin=0.05:bomin=0.05"
    ```
